function updateLanguage(language) {
	switch (language) {
		case 'english':
			document.querySelector('#group').innerHTML = 'Group matching tabs';
			document.querySelector('#ungroup').innerHTML = 'Ungroup all tabs';
			document.querySelector('#lang').innerHTML = 'عربي';
			break;
		case 'arabic':
			document.querySelector('#group').innerHTML = 'تجميع علامات التبويب المطابقة';
			document.querySelector('#ungroup').innerHTML = 'فك تجميع كل علامات تبويب';
			document.querySelector('#lang').innerHTML = 'English';
			break;
	}
}
...
chrome.storage.local.get(['language'], (result) => {
	if (result.language == undefined) {
		// if there is none (first time opening extension), set it to english
		chrome.storage.local.set({ language: 'english' });
	} else {
		// update button text to language
...
document.querySelector('#lang').addEventListener('click', () => {
	// console.log(document.querySelector('#lang').innerHTML);
	let newLang = buttonToLang[document.querySelector('#lang').innerHTML];
	// console.log(newLang);
